...
class Relatorios():
    ...
    def geraRelatConcurso(self):
        ...
        self.c.showPage()
        self.c.save()
    def exportandoPDF(self):
        pdf = FPDF()
        pdf.add_page()
        pdf.set_font("Arial", size=18)
        pdf.cell(200, 10, txt="Lista de Jogos", ln=1, align='C')
        pdf.cell(200, 10, txt="Que você tenha muita sorte no próximo sorteio!!", ln=2, align='C')
        f = open('Arquivo_exportado.txt', 'r')
        pdf.set_font("Arial", size=12)
        for x in f:
            pdf.cell(150, 10, txt=x, ln=3, align='C')
        pdf.output('Arquivo_exportado' + ".pdf")
        webbrowser.open("Arquivo_exportado.pdf")
...
class Funcoes():
    ...
    def sortear(self):
        lista = list()
        self.jogos = list()
        qjs = int(self.sortear_entry.get())
        tot = 1
        while tot <= qjs:
            cont = 0
            while True:
                ...
                if cont >= 15:
                    break
            lista.sort()
            self.jogos.append(lista[:])
            lista.clear()
            tot += 1
        arquivo = open('Arquivo_exportado.txt', 'w')

        for i, l in enumerate(self.jogos):

            self.listajogos.insert(END, f'Jogo {i + 1}: {l}')

            arquivo.write(f'Jogo {i + 1}: {l} \n\n')

            sleep(0)
        #self.final2.set("JOGOS GERADOS COM SUCESSO \n \n" "CLIQUE EM GERAR PDF \n\n" "BOA SORTE \n \n")
        ...
        #for i, l in enumerate(jogos):
            #arquivo.write(f'Jogo {i + 1}: {l} \n\n')
        #arquivo.write("Boooaaa Sooorte!!!")
    def limpajogos(self):
        self.listajogos.delete(0,END)
...
class Application(Funcoes,Relatorios):
    ...
    def Tela(self):
        ...
        self.root.geometry(newGeometry="1500x700")
        self.root.resizable(True, True)
        #self.root.state("zoomed")
        self.root.maxsize(width=2500, height=2500)
    ...
    def widgets_frame1(self):
        ...
        # Botão Comparar jogos com banco de dados
        self.bt_comparar = Button(self.aba3, text="Comparar", bd=2, bg='#1C1C1C', fg='white', font=('verdana', 8, 'bold'),
                                 command=self.sortear)
        self.bt_comparar.place(relx=0.2, rely=0.3, relwidth=0.1, relheight=0.15)

        # Botão Gerar PDF
        self.bt_gerarpdf = Button(self.aba3, text="Gerar PDF", bd=2, bg='#1C1C1C', fg='white', font=('verdana', 8, 'bold'),
                                 command=self.exportandoPDF)
        self.bt_gerarpdf.place(relx=0.085, rely=0.5, relwidth=0.1, relheight=0.15)

        # Botão Limpar
        self.bt_limpar = Button(self.aba3, text="Limpar", bd=2, bg='#1C1C1C', fg='white', font=('verdana', 8, 'bold'),
                                command=self.limpajogos)
        self.bt_limpar.place(relx=0.085, rely=0.7, relwidth=0.1, relheight=0.15)

        # Label de Apresentação
        self.lb_result = Label(self.aba3,
                               text="""Lotofácil sendo Fácil""",
                               bd=2, bg='#1C1C1C', fg='white', font=('verdana', 8, 'bold'), anchor='nw',
                               justify='center')
        self.lb_result.place(relx=0.6, rely=0)




        self.listajogos = Listbox(self.aba3,bg='#1C1C1C', fg='white', font=('verdana', 12, 'bold'),justify='center')
        self.listajogos.place(relx=0.37, rely=0.1, relwidth=0.6, relheight=0.4)
        self.scroolJogos = Scrollbar(self.aba3, orient='vertical')
        self.scroolJogos.place(relx=0.971, rely=0.1, relwidth=0.02, relheight=0.4)
        self.listajogos.configure(yscroll=self.scroolJogos.set)

        self.listaCompar = Listbox(self.aba3, bg='#1C1C1C', fg='white', font=('verdana', 12, 'bold'), justify='center')
        self.listaCompar.place(relx=0.37, rely=0.5, relwidth=0.6, relheight=0.4)
        self.scroolCompar = Scrollbar(self.aba3, orient='vertical')
        self.scroolCompar.place(relx=0.971, rely=0.5, relwidth=0.02, relheight=0.4)
        self.listaCompar.configure(yscroll=self.scroolCompar.set)
